...
class Osintgram:
    ...
    def __init__(self, u, p, settings):
        self.cli_mode = True
        self.u = u
        self.p = p
        self.login(u, p, settings)
        self.writeFile = True
        self.jsonDump = True
    ...
    def setTarget(self, target):
        ...
        self.followers = self.check_followers()
    ...
    def get_fwers_info(self, con):
        ...
        if con is not None:
            datas = self.get_info_db(con)


            for follower in followers:
                try:
                    cur = con.cursor()
                    if len(follower['full_name']):
                        sql = f"INSERT INTO public.followers (user_id, id,username,fullname) VALUES ({datas['pk']},{follower['id']},'{follower['username']}','{follower['full_name']}')"
                    else:
                        sql = f"INSERT INTO public.followers (id,username) VALUES ({datas['pk']},{follower['id']},'{follower['username']}')"
                    print(sql)
                    cur.execute(sql)
                    cur.close()
                    con.commit()
            ...
            for email in emails:
                try:
                    cur = con.cursor()
                    sql = f"INSERT INTO public.emails (id,email) VALUES ({email['id']},'{email['email']}')"
                    print(sql)
                    cur.execute(sql)
                    cur.close()
                    con.commit()

                    print(f"New email: {email['username']} - {email['email']}")
                except (Exception, psycopg2.errors.UniqueViolation):
                    cur.close()
                    con.commit()
                    pass
            ...
            for phone in phones:
                try:
                    cur = con.cursor()
                    sql = f"INSERT INTO public.phones (id,phone) VALUES ({phone['id']},'{phone['contact_phone_number']}')"
                    print(sql)
                    cur.execute(sql)
                    cur.close()
                    con.commit()

                    print(f"New phone: {phone['username']} - {phone['contact_phone_number']}")
                except:
                    cur.close()
                    con.commit()
                    pass
    ...
    def change_target_osintgram(self, line):
        self.setTarget(line)
        return
    ...
    def get_followers_te(self, test, email_phone):
        if self.check_private_profile():
            return
        ...
        if email_phone == True:
            ...
            for follow in followers:
                    ...
                    sys.stdout.write("\rCatched %i followers phone numbers" % len(results_u))
                    sys.stdout.flush()
                    ...
                    if 'contact_phone_number' in user['user'] and user['user']['contact_phone_number']:
                        ...
                        if test == True:
                            ...
                            results_u.append(follow)
        return results_u


                    # elif 'public_email' not in user['user'] and user['user']['public_email']:
                    #     follow['email'] = ''
        ...
        #         if 'contact_phone_number' in user['user'] and user['user']['contact_phone_number']:
        #             follow['contact_phone_number'] = user['user']['contact_phone_number']
        #         # elif 'contact_phone_number' not in user['user'] and user['user']['contact_phone_number']:
        #         #
        #         #     follow['contact_phone_number'] = ''
        #
        ...
        #         sys.stdout.write("\rCatched %i followings email" % len(results_u))
        #         sys.stdout.flush()
        #         user = self.api.user_info(str(follow['id']))
        #         if 'public_email' in user['user'] and user['user']['public_email']:
        #             follow['email'] = user['user']['public_email']
    ...
    def get_followers(self, test, email_phone):
        if self.check_private_profile():
    ...
    def get_followers_db(self, acc, conn):
        if self.check_private_profile():
            return
        ...
        for user in _followers:
            try:
                cur = conn.cursor()
                if len(user['full_name']):
                    sql = f"INSERT INTO public.followers (id,f_id,f_username,f_fullname) VALUES ({acc.id},{user['pk']},'{user['username']}','{user['full_name']}')"
                else:
                    sql = f"INSERT INTO public.followers (id,f_id,f_username) VALUES ({acc.id},{user['pk']},'{user['username']}')"
                # print(sql)
                cur.execute(sql)
            except (Exception, psycopg2.errors.UniqueViolation):
                pass
    ...
    def get_info_db(self, conn):
        ...
        try:
            cur = conn.cursor()
            sql = f"INSERT INTO public.users (id,username,fullname) VALUES ({data['pk']},'{self.target}','{data['full_name']}')"
            cur.execute(sql)
        except (Exception, psycopg2.errors.UniqueViolation):
            pass
    ...
    def get_user_for_comments(self, users):
        people = []
    ...
    def get_user_for_list(self, users):
        people = []
    ...
    def get_us_stories(self):
        ...
        if data['items'] is not None:  # no stories avaibile
            for i in data['items']:
                ...
                counter = counter + 1
        return paths
    def get_user_stories(self, limit):
        if self.check_private_profile():
            return
    ...
    def login(self, u, p, cached_settings):
        try:
            if not len(cached_settings):
